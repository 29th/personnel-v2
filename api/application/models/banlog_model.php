...
class Banlog_model extends MY_Model {
    ...
    public function search_roid($seek_line) {
        ...
        $this->db->having("banlog.roid LIKE '%$esc_str%' OR banlog.uid LIKE '%$esc_str%' OR banlog.guid LIKE '%$esc_str%' OR banlog.handle LIKE '%$esc_str%' OR banlog.reason LIKE '%$esc_str%' OR banlog.comments LIKE '%$esc_str%' OR banlog.ip LIKE '%$esc_str%'");
    }
}