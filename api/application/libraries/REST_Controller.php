...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function response($data = null, $http_code = null)
	{
        ...
        if($cors = $CFG->item('cors')) {
            if(is_array($cors)) {
                foreach($cors as $origin) {
                    if($origin == $this->input->server('HTTP_ORIGIN')) {
                        log_message('debug', 'Good origin: ' . $this->input->server('HTTP_ORIGIN') . ' - ' . $this->input->server('HTTP_REFERER'));
                        header('Access-Control-Allow-Origin: ' . $origin);
                        break;
                    } else log_message('debug', 'Bad origin: ' . $this->input->server('HTTP_ORIGIN') . ' - ' . $this->input->server('HTTP_REFERER'));
                }
            } else {
        ...
        if($cors_credentials = $CFG->item('cors_credentials')) {
            header('Access-Control-Allow-Credentials: ' . $cors_credentials);
        }
        
        if($cors_headers = $CFG->item('cors_headers')) {
            header('Access-Control-Allow-Headers: ' . $cors_headers);
        }
        
        // Client-side caching
        if($client_side_caching = $CFG->item('client_side_caching')) {
            header('Cache-Control: max-age=' . $client_side_caching);
        }
