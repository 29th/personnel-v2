    ...
    private function tpAddMemberEnlistment( $unit_id, $member_id ) {
        $cSql = "
            SELECT e.id, e.status, 
                ...
                (SELECT CONCAT(ru.abbr,' - ',ru.game,'/',ru.timezone) FROM assignments AS ra LEFT JOIN units AS ru ON ra.unit_id = ru.id WHERE ra.end_date IS NULL AND ru.classification='Combat' AND ra.member_id = e.recruiter_member_id ORDER BY (ru.path) DESC LIMIT 1) AS `recruiter|assignement`
" . /*                poa_result, 
                passed */ "
